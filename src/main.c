...
int main(int argc, char **argv) {
  ...
  window = init_window(DEFAULT_TEXTURE_WIDTH, DEFAULT_TEXTURE_HEIGHT,
                       "Hayware Engine");
  ...
  GLuint vertexShader = compileShader("../src/shaders/"
                                      "vertexShader.vert",
                                      Vertex_Shader);
  GLuint fragmentShader = compileShader("../src/shaders/"
                                        "fragShader.frag",
                                        Fragment_Shader);
  // -------

  // Used to display the rendered texture to the screen
  GLuint vertexShaderQuad = compileShader("../src/shaders/"
                                          "vertexShaderQuad.vert",
                                          Vertex_Shader);
  GLuint fragmentShaderQuad = compileShader("../src/shaders/"
                                            "fragShaderQuad.frag",
                                            Fragment_Shader);
  // -------
  ...
  GLuint shaderProgramQuad =
      createProgram(vertexShaderQuad, fragmentShaderQuad);
  // -------

  SDL_GL_SetSwapInterval(0); // Uncomment to disable VSYNC

  // Create textures and FBOs
  BREAKOUT_TEXTURE FBOtexture[2] = {createBreakoutTexture(),
                                    createBreakoutTexture()};

  GLuint FBO[2] = {createBreakoutFBO(FBOtexture[0]),
                   createBreakoutFBO(FBOtexture[1])};
  // -------
  ...
  while (running) {
    ...
    while (SDL_PollEvent(&event)) {
      ...
      case SDL_EVENT_MOUSE_BUTTON_DOWN:
        ...
        mouseY = DEFAULT_TEXTURE_HEIGHT -
                 event.button.y; // Make Y go from 0 (bottom) to 800 (top)
        mouseIsPressed = true;
        ...
        // Debugging info
        // printf("Sand Spawn Radius: %d\n", sandSpawnRadius);
        // printf("%d %d\n", mouseX, mouseY);

        // Send mouse position to the shader
        glUniform2i(glGetUniformLocation(shaderProgram, "mouseCoord"), mouseX,
                    mouseY);
        glUniform1i(glGetUniformLocation(shaderProgram, "mousePressed"),
                    1); // Mouse is pressed
        glUniform1i(glGetUniformLocation(shaderProgram, "spawnRadius"),
                    sandSpawnRadius);
        break;
    ...
    // Mouse is being held and the position has not changed (allow sand to fall
    // from a single point)
