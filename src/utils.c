...
void createFullscreenQuad(VECTOR *vertices, VECTOR *indices) {
  VERTEX topLeft = {
      {-1.0f, 1.0f, 0.1f}, {1.0f, 1.0f, 1.0f}, {0.0f, 1.0f, 0.0f}};
  VERTEX topRight = {
      {1.0f, 1.0f, 0.1f}, {1.0f, 1.0f, 1.0f}, {1.0f, 1.0f, 0.0f}};
  VERTEX bottomRight = {
      {1.0f, -1.0f, 0.1f}, {1.0f, 1.0f, 1.0f}, {1.0f, 0.0f, 0.0f}};
  VERTEX bottomLeft = {
      {-1.0f, -1.0f, 0.1f}, {1.0f, 1.0f, 1.0f}, {0.0f, 0.0f, 0.0f}};
...
void push_back(VECTOR *v, void *element) {
  ...
  memcpy((char *)v->array + (v->currentIndex * v->dataSize), element,
         v->dataSize);
  (v->currentIndex)++;
}
...
GLdouble toRadians(GLdouble degrees) { return (degrees * (PI / 180)); }
